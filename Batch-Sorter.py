...
while i < Number_Of_Possible_Batches:
      ...
      with open(json_save_path + ("/Batch{}".format(i+1)), "w") as outfile:
            outfile.write(batchDictionaryObject)
      i += 1

incompleteBatch = {}
incompleteBatch["NFTs_in_Batch"] = int(len(BatchDNAList))
incompleteBatch["variantMetaData"] = variantMetaData
incompleteBatch["BatchDNAList"] = BatchDNAList

incompleteBatch = json.dumps(incompleteBatch, indent=1, ensure_ascii=True)

with open(json_save_path + ("/Batch{}".format(i+1)), "w") as outfile:
      outfile.write(incompleteBatch)
