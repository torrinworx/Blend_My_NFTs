...
def returnData():
        ...
        for i in attributeVariants:
            ...
            if len(orderRarity) == 0:
                ...
                print(
                    "The collection "
                    + str(i)
                    + " doesn't follow the naming conventions of attributes. Please move this \n"
    ...
    def numOfCombinations(hierarchy):
        ...
        if combinations == 0:
            ...
            print("")
            print(
                "Here is the hierarchy of all collections the DNA_Generator gathered from your .blend file, excluding "
        ...
        if numBatches < 1:
            ...
            print(
                "The number of NFTs Per Batch (nftsPerBatch variable in config.py) is to high."
            )
            print(
                "There are a total of "
                + str(combinations)
                + " possible NFT combinations and you've requested "
                + str(config.nftsPerBatch)
                + " NFTs per batch."
            )
            print(
                "Lower the number of NFTs per batch in config.py or increase the number of \nattributes and/or variants"
                " in your .blend file."
            )
...
def generateNFT_DNA():
    ...
    print("")
    print(
        "Generating " + str(config.maxNFTs) + " combinations of DNA. Set in config.py."
    )
    print("")
    ...
    if not config.enableRarity:
        ...
        def createDNARandom():
            ...
            if config.checkDNARules:
                isDnaValid, newDNA = CheckDNARules.CheckDNA(dna, hierarchy)
                dna = newDNA
                if not isDnaValid:
                    print("Recrate valid DNA")
                    return createDNARandom()
        ...
        if config.nftsPerBatch > config.maxNFTs:
            ...
            print(
                "The Max num of NFTs you chose is smaller than the NFTs Per Batch you set. Only "
                + str(config.maxNFTs)
                + " were added to 1 batch"
            )

    if config.enableRarity:
        print(
            bcolors.OK
            + "Rarity is on. Weights listed in .blend will be taken into account "
            + bcolors.RESET
        )
        possibleCombinations = config.maxNFTs
...
def send_To_Record_JSON():
    ...
    print("")
    print(
        "All possible combinations of DNA sent to NFTRecord.json with "
        + str(possibleCombinations)
        + " NFT DNA sequences generated in %.4f seconds" % (time.time() - time_start)
    )
    print("")
    print(
        "If you want the number of NFT DNA sequences to be higher, please add more variants or attributes to your .blend file"
    )
    print("")
